...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    for i in range(0, len(corners)):
        if visited[i]:
            continue
        cornerDistance = manhattanDistance(state[0],
                                           corners[i])  # Not visited, so find the distance form the current state
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    for i in range(len(foodpos)):  # it expands only 4100 and something nodes).We can do the same with
        fcost[i] = mazeDistance(pos, foodpos[i],
                                problem.startingGameState)  # the cornersheuristis using manhattanheuristic but for food and corners
    Max = max(fcost)
    return Max
